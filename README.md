...
- Return HTTP status 400 for an invalid request with a response 
...
- Return the HTTP status 200 if any documents are found.
- If the book has no reviews then the response body should include book detail and an empty array for reviewsData.
- If no documents are found then return an HTTP status 404 with a response 
...
- Check if the review exist with the reviewId. Check if the book exist with the bookId. Send an error response with appropirate status code if the book or book review does not exist
